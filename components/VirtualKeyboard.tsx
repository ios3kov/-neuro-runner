...
interface VirtualKeyboardProps {
  ...
  onNext: () => void;
  onClose: () => void;
  showSubmit: boolean; // If true, show ENTER, else show NEXT
}
...
export const VirtualKeyboard: React.FC<VirtualKeyboardProps> = ({
  ...
  const handleTap = (action: () => void) => {
    ...
    haptics.impactLight(); // Vibration
    action();
  };

  return (
    <div className="fixed bottom-0 left-0 w-full bg-black/95 border-t-2 border-cyan-500/50 backdrop-blur-xl z-[9999] pb-safe animate-in slide-in-from-bottom duration-300 select-none touch-none">
